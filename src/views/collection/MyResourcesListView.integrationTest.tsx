...
describe('MyResourcesListView', () => {
  it('when user has no bookmarks or collections, it renders a helper message', () => {
    ...
    expect(getByText('You don\'t have any saved resources, yet.')).toBeInTheDocument();
  });
  ...
  it(`only show my collections and bookmarked collections`, () => {
    ...
    const bookmarkedCollection = getCollectionWithData('123');
    ...
    const { queryByText } = renderWithStore(<MyResourcesListView />, {
      initialState: {
        ...
        collections: CollectionsFactory.sample({
          myResources: { items: [myCollection.id], links: {} },
        }),
      },
    });
    ...
    //change setup to be myresources rather than old mycollections. will then strip out all the my collection stuf
  ...
  it('when user deletes a collection, it is no longer showed in the page', async () => {
    ...
    // Get one to delete
    const collectionToBeDeleted = myCollectionsBeforeDelete[1];
    const collectionNotToBeDeleted = myCollectionsBeforeDelete[0];
